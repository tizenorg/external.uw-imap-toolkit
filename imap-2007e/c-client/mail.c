...
#endif
{
  ...
  if(NIL == elt)  /*Fix for H0100087205. [santosh.br@samsung.com] */
  {
  	return NIL;
  }
...
if(iDownload==1)
{
 ...
 // 	fprintf(stderr, "free env and body and evn incomplete [%d] \n",  (*env)->incomplete);

    mail_free_envelope (env);	/* flush old envelope and body */
    mail_free_body (b);
				/* see if need to fetch the whole thing */
    if (body || !elt->rfc822_size) {
//		fprintf(stderr, "fetch structuree - 2\n");
...
//		fprintf(stderr, "fetch structuree - 3\n");
			  ...
			  unsigned long flags)
{
  NETSTREAM *stream = NIL;
  void *tstream;
	/* Removed for timeout on SSL connection */
  /* if (service && (*service == '*')) { */
